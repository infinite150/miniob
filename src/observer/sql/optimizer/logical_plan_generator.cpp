...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<LogicalOperator> update_oper(
      new UpdateLogicalOperator(table, update_stmt->field_metas(), &update_stmt->rhs_expressions()));
...
RC LogicalPlanGenerator::create_group_by_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // collect aggregate expressions from SELECT list and HAVING
  for (unique_ptr<Expression> &expression : query_expressions) {
...
{
  logical_operator = nullptr;
  vector<unique_ptr<Expression>> &order_exprs = select_stmt->order_by();
  if (order_exprs.empty()) {
    return RC::SUCCESS;
  }
  vector<bool> &asc = select_stmt->order_by_asc();
  if (asc.size() != order_exprs.size()) {
    LOG_WARN("order by asc flags size mismatch");
    return RC::INVALID_ARGUMENT;
  }
  logical_operator = make_unique<OrderByLogicalOperator>(std::move(order_exprs), std::move(asc));
  return RC::SUCCESS;
}
