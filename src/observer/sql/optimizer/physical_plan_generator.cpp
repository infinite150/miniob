...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper, Session *session)
{
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(
      update_oper.table(), update_oper.field_metas(), update_oper.rhs_expressions(), session));
...
RC PhysicalPlanGenerator::create_plan(TableGetLogicalOperator &table_get_oper, unique_ptr<PhysicalOperator> &oper, Session* session)
{
  ...
  if (!table_get_oper.alias().empty()) { table_scan_oper->set_table_alias(table_get_oper.alias()); }
...
RC PhysicalPlanGenerator::create_plan(OrderByLogicalOperator &logical_oper, unique_ptr<PhysicalOperator> &oper, Session *session)
{
  ...
  for (unique_ptr<Expression> &expr : logical_oper.order_exprs()) {
    ExpressionIterator::for_each_subquery(*expr, [session](SubQueryExpr &sq) {
      sq.generate_logical_oper();
      sq.generate_physical_oper(session);
    });
  ...
  auto order_phy = make_unique<OrderByPhysicalOperator>(std::move(logical_oper.order_exprs()), std::move(logical_oper.asc_flags()));
